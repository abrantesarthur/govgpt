import { DEPUTY_EXPENSES_CSV_HEADER_KEYS, DEPUTY_EXPENSES_FILE_PATH } from "../constants";
...
export const buildDeputyExpensesFile = async () => {
    ...
    const csvHeader = `${DEPUTY_EXPENSES_CSV_HEADER_KEYS.join(',')}\n`;
    fs.writeFileSync(DEPUTY_EXPENSES_FILE_PATH, csvHeader);
    ...
    for (let i = 0; i < deputies.length; i += 1) {
        ...
        const expensesCsv = expenses.map((expense) => {
            ...
            return DEPUTY_EXPENSES_CSV_HEADER_KEYS.map((k) => mergedDeputyExpense[k] ?? '').join(',') + '\n';
        }).join('');

        // write the expenses to the CSV file
        fs.appendFileSync(DEPUTY_EXPENSES_FILE_PATH, expensesCsv, {});
